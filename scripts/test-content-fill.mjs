...
assert.doesNotMatch(content, /KeyboardEvent/);
assert.doesNotMatch(content, /keyCode:\s*27/);
assert.doesNotMatch(content, /document\.dispatchEvent\(new KeyboardEvent\("keydown"/);
const closer = content.slice(content.indexOf("function closeOpenListbox"), content.indexOf("function pickOpenOption"));
assert.ok(closer.includes("function closeOpenListbox"), "closeOpenListbox must exist");
assert.doesNotMatch(closer, /key:\s*["']Escape["']/);
assert.doesNotMatch(closer, /dispatchEvent/);
assert.match(closer, /aria-expanded="true"/);
...
assert.match(fillOnce, /typeStuck/);
assert.match(fillOnce, /vehicleTypeLooksSet/);
assert.match(fillOnce, /if \(!typeStuck\)/);
